...
import useGetClientQuery from '../../hooks/queries/clients/useGetClientQuery';

const EditClientForm = () => {
  const { t } = useTranslation();
  const {
    state: { editClientForm, editClientId },
  ...
  const {
    ...
    isLoading,
    isSuccess,
  } = useGetClientQuery(editClientId as number);

  React.useEffect(() => {
    if (isSuccess && editClientFormData?.name) {
      updateClientsState({
        type: EClientsActionType.setClientFormData,
        payload: editClientFormData,
      });
    }
  }, [editClientFormData?.name]);
        ...
        case EFormFieldType.input:
          ...
          if (format) {
            if (fieldEditValue.match(format)) {
              updateClientsState({
                type: EClientsActionType.editClientForm,
            ...
            <FormControlLabel
              ...
              control={
                <Checkbox
                  ...
                  size={size ?? 'small'}
                  // @ts-ignore
                  checked={editClientForm[name] as boolean}
